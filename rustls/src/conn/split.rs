...
use crate::msgs::{Delocator, TlsInputBuffer};
use crate::sync::Arc;
...
///
/// Each call to [`ReceiveTraffic::read()`] consumes the `ReceiveTraffic` and
/// returns one of these states. Every state except [`CloseNotify`] gives the
/// `ReceiveTraffic` back so you can call `read()` again; note that each does so
...
///        ├─────────────╯      │       │   │ terminal │
///        │                    │       │   ╰──────────╯
///        │         WakeSender │       │ Available
///        │    .into_next() -> │       │ .into_next() ->
///        │     ReceiveTraffic │       │ (discard, ReceiveTraffic)
///        ╰────────────────────┴───────╯
/// ```
///
/// - [`Await`]: more TLS input is required. The variant holds the
///   `ReceiveTraffic` directly - call [`read()`] again once
///   you have more input.
/// - [`WakeSender`]: receiving may have produced data to send. Call
///   [`WakeSender::into_next()`] to resume receiving.
/// - [`Available`]: application data was received. Consume the bytes, then call
///   [`ReceivedApplicationData::into_next()`] to obtain the discard count and
///   resume receiving.
/// - [`CloseNotify`]: the peer closed the receive direction cleanly.
///   The `ReceiveTraffic` is consumed.
///
/// [`read()`]: ReceiveTraffic::read
/// [`Await`]: ReceiveTrafficState::Await
/// [`WakeSender`]: ReceiveTrafficState::WakeSender
